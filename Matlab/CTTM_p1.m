...
% Earth inertial longitude at departure
theta1 = wrapToPi(f1 + AOP1); 

% Spacecraft arrives 180° ahead of theta1
theta2 = wrapToPi(theta1 + pi); 

% Mars angles at ARRIVAL
% ARRIVAL true anomaly from periapsis 
f2_arrival = wrapToPi(theta2 - AOP2); % Where Mars should be when satellite arrives

% ARRIVAL true longitude (this is what the plot shows)
thetaMars_arrival = wrapToPi(AOP2 + f2_arrival);
...
p2 = a2*(1-e2^2); 
r1 = p1/(1+e1*cos(f1));
r2 = p2/(1+e2*cos(f2_arrival)); % Mars position when satellite arrives
l = r1+r2; 
m = r1*r2*(1+cos(df));
k = r1*r2*(1-cos(df)); 
...
% --- Propagate Mars back by TOF to get DEPARTURE angles -
% Calculate Mars true anomaly at departure (f2_start)

E2_arrival = 2*atan2(tan(f2_arrival/2),sqrt((1+e2)/(1-e2)));
M2_arrival = wrapToPi(E2_arrival-e2*sin(E2_arrival)); 
M2_departure = wrapToPi(M2_arrival-sqrt(mu/a2^3)*TOF); 
E2_departure = Newtonraphson(M2_departure, e2);
f2_start = 2*atan2(sqrt((1+e2)/(1-e2)) *tan(E2_departure/2),1);

% True LONGITUDE of Mars at departure (inertial)
thetaMars_start = wrapToPi(AOP2 + f2_start);

% True (longitudes) 
fprintf('Mars true longitude at departure: %.4f degrees\n', wrapTo360(rad2deg((thetaMars_start))));
fprintf('Mars true longitude at arrival:   %.4f degrees\n', wrapTo360(rad2deg((thetaMars_arrival))));

% for true anomaly on mars' orbit, just subtract the AOP

% Phase angle (lead or lag)
dtheta = wrapToPi(thetaMars_start - theta1);
if dtheta >= 0
    fprintf('Mars leads Earth at Departure by %.4f degrees\n', wrapTo360(rad2deg(dtheta)));
else
    fprintf('Mars lags Earth at Departure by  %.4f degrees\n', wrapTo360(abs(rad2deg(dtheta))));
end
...
v_peri_t = [-sqrt(mu/p)*sin(f1); sqrt(mu/p)*(e_transfer+cos(f1)); 0];
% Rotate perifocal -> ECI with (RAANt, IT, AOPt)
R1 = [cos(RAANt), -sin(RAANt), 0; sin(RAANt),  cos(RAANt), 0; 0,0,1];
R2 = [1, 0,0; 0, cos(IT), -sin(IT); 0, sin(IT), cos(IT)];
R3 = [cos(AOPt), -sin(AOPt), 0; sin(AOPt),  cos(AOPt), 0; 0,0,1];
DCM_t = R1*R2*R3;
vt_vec = DCM_t * v_peri_t;
