...
synodic_period_days = TimeDelta(
    1 / abs((1 / (earth_period/86400)) - (1 / (mars_period/86400))), format='jd')
...
for tof_days in tof_range:
    ...
    r1_earth_bary, v1_earth_bary = get_body_barycentric_posvel(
        'earth', departure_date)
    r2_mars_bary, v2_mars_bary = get_body_barycentric_posvel(
        'mars', departure_date+tof)
    ...
    v1_earth = (v1_earth_bary.xyz - v_sun1.xyz).to(u.km /
    ...
    transfer_angle = (np.acos(np.dot(r1_earth, r2_mars) /
                      (np.linalg.norm(r1_earth) * np.linalg.norm(r2_mars))))
    ...
    transfer_short.a, transfer_short_p, transfer_short.e, transfer_v1_short, transfer_v2_short = universal_lambert(
    ...
    transfer_long.a, transfer_long_p, transfer_long.e, transfer_v1_long, transfer_v2_long = universal_lambert(
    ...
    # arrival hyperbolic excess velcity in the MCI frame
    Vinf_arrival = (transfer_v2 - v2_mars)
    # departure hyperbolic excess velociy in the ECI frame
    Vinf_departure = (transfer_v1 - v1_earth)
...
def find_optimal_solution(results, weight_C3, weight_Vinf):
    ...
    C3_norm = (C3_values - np.min(C3_values)) / \
        (np.max(C3_values) - np.min(C3_values))
    Vinf_norm = (Vinf_mag - np.min(Vinf_mag)) / \
        (np.max(Vinf_mag) - np.min(Vinf_mag))
...
optimal_solution = find_optimal_solution(
    results, weight_C3=0.75, weight_Vinf=0.25)
...
def vinf_to_raan_dec(Vinf):
    ...
    print(
        f'RAAN: {np.degrees(RAAN): .3f}° | Declination: {np.degrees(Dec): .3f}°')
...
print(
    f'Outbound RAAN = {np.degrees(RAAN_dep): .3f}° | Outbound Declination = {np.degrees(Dec_dep): .3f}°\n')
...
earth_parking = Orbit(mu=EARTH_MU, a=(200+Earth_rad)*u.km, e=0.0*u.km/u.km, f0=(0*u.rad),
                      inc=(28.5*u.deg).to(u.rad), raan=(RAAN_dep*u.deg).to(u.rad), aop=(0*u.deg).to(u.rad))
earth_parking.p = earth_parking.p
...
r_pqw, v_pqw = orb_2_pqw(r.value, sat_orbit.f0.value,
                         sat_orbit.e.value, sat_orbit.p.value, sat_orbit.mu.value)
# converts perifocal frame to eci frame
r_eci, v_eci = perif_2_eci(
    r_pqw, v_pqw, sat_orbit.inc.value, sat_orbit.raan.value, sat_orbit.aop.value)
sat.r0 = r_eci * u.km
...
# --> velocity needed at perigee of the hyperbolic escape trajectory. this velocity is wrt Earth
v_p_hyp = np.sqrt(
    2*(hyperbolic_energy + (EARTH_MU/np.linalg.norm(sat.r0.value)).value))
# --> required deltaV to get vinf.
delta_V = v_p_hyp - np.linalg.norm(sat.v0.value)
print(
    f'Required Delta V to achieve a Vinf of {C3:.3f} at departure is {delta_V:.3f} km/s\n)')
...
# --> this velocity is also WRT and is the new departure velcocity after dv is applied
v_post_deltaV = v_eci + (delta_V * v_hat)

# from here, we can calculate what v_inf departure is, ie C3
# --> this is the vinf in the eci framee
v_inf_departure = v_post_deltaV - v1_earth
...
r_sats, _, _ = propagate_rk4(
    r1_earth, transfer_v1, departure_date, arrival_date, dt, fun_arg=fun_arg)
